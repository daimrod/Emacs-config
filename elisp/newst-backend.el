...
(defun newsticker--get-alternate-link (node)
  "Return alternate link in NODE."
  (let (link-alternate rel)
    (cl-loop for link in (xml-get-children node 'link)
             for rel = (xml-get-attribute-or-nil link 'rel)
             if (or (null rel)
                    (string= "alternate" rel))
             return link)))
...
same as in `newsticker--parse-atom-1.0'."
  ...
  (let (new-feed new-item)
    ...
    (setq new-item (newsticker--parse-generic-items
                    ...
                    (lambda (node)
                      (xml-get-attribute
                       (newsticker--get-alternate-link node) 'href))
                    ;; time-fn
                    (lambda (node)
...
or URL `http://my.netscape.com/publish/formats/rss-spec-0.91.html'."
  ...
  (let* ((channelnode (car (xml-get-children topnode 'channel)))
    ...
    (setq has-new-items (newsticker--parse-generic-items
                         ...
                         (lambda (node)
                           (car (xml-node-children
                                 (newsticker--get-alternate-link node))))
                         ;; time-fn
                         (lambda (node)
                           (newsticker--decode-rfc822-date
                            (car (xml-node-children
